...
const Resources = ({ isLoggedIn }) => {
  ...
  const handleDownload = (resource) => {
    if (resource.is_premium && !isLoggedIn) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-luxury-pearl">
        ...
        >
          {categories.map(category => (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-full font-montserrat font-medium transition-all duration-200 ${selectedCategory === category.id
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between text-sm font-montserrat text-gray-500 mb-4">
                  <span>{resource.type} • {resource.size || 'N/A'}</span>
                  <span>{resource.downloads_count?.toLocaleString() || 0} downloads</span>
                </div>
                <button
                  onClick={() => handleDownload(resource)}
                  className={`w-full py-3 rounded-lg font-montserrat font-medium transition-all duration-200 flex items-center justify-center ${resource.is_premium && !isLoggedIn
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                    : 'bg-gold-gradient text-navy-800 hover:shadow-lg'
                    }`}
                  disabled={resource.is_premium && !isLoggedIn}
                >
                  <SafeIcon icon={FiDownload} className="w-5 h-5 mr-2" />
                  {resource.is_premium && !isLoggedIn ? 'Sign In Required' : 'Download'}
                </button>
              </div>
            </motion.div>
          ))}
