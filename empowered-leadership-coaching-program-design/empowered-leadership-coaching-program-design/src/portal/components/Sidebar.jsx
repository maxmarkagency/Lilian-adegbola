  ...
  const renderNavigationItem = (item, isBottomNav = false) => {
    ...
    return (
      <Link
        key={item.name}
        to={item.href}
        className={`flex items-center space-x-2 px-2 py-2 rounded transition-all duration-200 group ${isCurrentlyActive
      ...
      >
        <SafeIcon
          icon={item.icon}
          className={`w-4 h-4 flex-shrink-0 ${isCurrentlyActive
            ? 'text-white'
        ...
        {!isCollapsed && (
          <div className="flex-1 min-w-0">
            <p className="font-montserrat font-medium text-xs truncate">{item.name}</p>
            <p className={`text-xs hidden sm:block truncate ${isCurrentlyActive
              ? 'text-gold-100'
              : 'text-gray-500'
              }`}>
              {item.description}
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`fixed left-0 top-0 h-full bg-white shadow-lg z-50 flex flex-col lg:relative ${isCollapsed ? 'w-12' : 'w-60'
          }`}
      >
        {/* Header */}
