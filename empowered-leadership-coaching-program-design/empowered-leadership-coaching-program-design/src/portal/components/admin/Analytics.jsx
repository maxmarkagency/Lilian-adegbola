...
const Analytics = () => {
  ...
  const [stats, setStats] = useState({
    revenue: 0,
    activeUsers: 0,
    courseCompletions: 0, // Mocked/Placeholder
  ...
  // Partially real analytics data
  const overviewStats = [
    {
      title: 'Total Revenue',
      value: `$${stats.revenue.toLocaleString()}`,
      change: '+18.5%', // Mocked trend
      trend: 'up',
      icon: FiDollarSign,
      color: 'emerald'
    },
              ...
              {deviceBreakdown.map((device, index) => (
                <div key={index} className="space-y-2">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${device.device === 'Desktop' ? 'bg-blue-500' :
              ...
              <tbody className="divide-y divide-gray-200">
                {topPages.map((page, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`font-montserrat ${parseInt(page.bounce) > 40 ? 'text-red-600' :
